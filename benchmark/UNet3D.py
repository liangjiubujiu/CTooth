...
class UNet3D(BaseModel):
    ...
    def __init__(self, in_channels, n_classes, base_n_filter=8,projector=False):
        ...
        self.projector_layer=Projj(self.n_classes)
    ...
    def forward(self, x,deep=False):
        ...
        #     final_projector = self.projector_layer(out)
        #
        #     return out_pred,final_projector
        # else:
        #     return out_pred

        dsv4 = self.ds2_1x1_conv3d(ds2)
        ds1_ds2_sum_upscale = self.upsacle(dsv4)
        dsv3 = self.ds3_1x1_conv3d(ds3)
        ds1_ds2_sum_upscale_ds3_sum = ds1_ds2_sum_upscale + dsv3
        dsv2 = self.upsacle(ds1_ds2_sum_upscale_ds3_sum)
        ...
        if self.projector:
            ...
            final_projector = self.projector_layer(final)

            if deep:
                return final, [dsv4, dsv3, dsv2, dsv1], final_projector
            else:
                return final, None, final_projector
        else:
            if deep:
                return final, [dsv4, dsv3, dsv2, dsv1], None
            else:
...
class Projj(nn.Module):
    def __init__(self, n_classes):
        super(Projj, self).__init__()
        self.dsv = nn.Sequential(nn.Conv3d(n_classes , n_classes , 1,1,0),
                                         nn.ReLU(inplace=True),
                                         nn.BatchNorm3d(n_classes),
                                         nn.Conv3d(n_classes , n_classes, 1) )
